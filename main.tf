...
resource "yandex_compute_instance_group" "lamp-group" {
  ...
  instance_template {
    ...
    metadata = {
      ...
      user-data = <<-EOF
        #!/bin/bash
        echo "<html><body><h1>Welcome to my website!</h1><p>Here is the image from the bucket:</p><img src='https://storage.yandexcloud.net/paint/goa.jpg'>>        systemctl restart apache2
      EOF
    }
  }
